...
with open("moviesFinal.json", "r", encoding="utf-8") as file:
    data = json.load(file)
...
for id, name in data["allPeople"].items():
    rdf_output += f"""
###  http://npcw/ontologies/2025/cinema/{id}
...
"""
    ...
    for person in movie["peopleInvolved"]:
        ...
        rdf_output += f"""
###  http://npcw/ontologies/2025/cinema/{person_id}
...
with open("populado.ttl", "w", encoding="utf-8") as ttl_file:
    ttl_file.write(rdf_output)

print("O ficheiro populado.ttl foi gerado com sucesso!")